class_name SearchTrainerBrain
extends TrainerBrain

const SOURCE_ID := &"simultaneous_search_composite_v1"

var profile: TrainerProfile
var last_trace: TrainerDecisionTrace = null

var _catalog: DefinitionCatalog
var _search: TrainerSimultaneousSearch
...
func _init(
	...
	brain_id = &"search_trainer_brain_v1"
	_search = TrainerSimultaneousSearch.new(_catalog, profile)
...
func choose_action(context: TrainerDecisionContext) -> BattleAction:
	...
	if best_action == null:
		...
		return null
	last_trace.select(best_action, "highest_robust_search_score")
	last_trace.metadata["selected_score"] = best_score
	last_trace.metadata["search_model"] = TrainerSimultaneousSearch.SEARCH_MODEL_ID
