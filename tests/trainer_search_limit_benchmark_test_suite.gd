class_name TrainerSearchLimitBenchmarkTestSuite
extends RefCounted
...
func run(check_callback: Callable) -> void:
	...
	_build_catalog()
	_test_limit_bank_contract()
	_test_search_limit_bank_is_reproducible()
...
func _build_catalog() -> void:
	...
	_add_species(
		H_CANDIDATE_SPECIES,
		40, 90, 70, 10,
		[H_SPEED_SETUP, H_ATTACK_SETUP, H_STRIKE],
	)
	_add_species(
		H_REFERENCE_SPECIES,
		56, 110, 70, 100,
	...
	_add_species(
		B_REFERENCE_SPECIES,
		50, 170, 70, 60,
		[B_WEAK_A, B_WEAK_B, B_WEAK_C, B_NUKE],
	)
	_add_species(
		U_CANDIDATE_SPECIES,
		40, 130, 50, 70,
	...
	# The hidden nuke is intentionally absent from the public level-up learnset.
	_add_species(
		U_REFERENCE_SPECIES,
		50, 170, 70, 60,
		[U_WEAK],
	...
	_check.call("limit_normal_type_available", _catalog.type(&"normal") != null)
	_check.call("limit_branch_nuke_is_public_prior", _species_learnset_has(B_REFERENCE_SPECIES, B_NUKE))
	_check.call("limit_hidden_nuke_not_public_prior", not _species_learnset_has(U_REFERENCE_SPECIES, U_HIDDEN_NUKE))


func _test_limit_bank_contract() -> void:
	...
	_check.call("limit_bank_has_three_scenarios", scenarios.size() == 3)
	_check.call("limit_bank_has_three_distinct_families", families.size() == 3)
	_check.call("limit_bank_runs_18_planner_matches", planner_matches == 18)
	_check.call("limit_bank_runs_12_oracle_matches", oracle_matches == 12)


func _test_search_limit_bank_is_reproducible() -> void:
	...
	_check.call("limit_bank_model_id", String(first.get("evaluation_model", "")) == TrainerSearchLimitBenchmark.MODEL_ID)
	_check.call("limit_bank_reports_three_scenarios", int(first.get("scenario_count", 0)) == 3)
	_check.call("limit_bank_planner_has_18_matches", int(totals.get("planner_matches", 0)) == 18)
	_check.call("limit_bank_oracle_has_12_matches", int(totals.get("oracle_matches", 0)) == 12)
	_check.call("limit_bank_no_planner_invalid", int(totals.get("planner_invalid", -1)) == 0)
	_check.call("limit_bank_no_oracle_invalid", int(totals.get("oracle_invalid", -1)) == 0)
	_check.call("limit_bank_signature_present", not String(first.get("signature", "")).is_empty())
	_check.call("limit_bank_is_deterministic", JSON.stringify(first) == JSON.stringify(second))

	var by_id := _scenario_map(first.get("scenarios", []) as Array)
	_test_horizon_limit(by_id)
	_test_branching_limit(by_id)
	_test_hidden_information_limit(by_id)


func _test_horizon_limit(by_id: Dictionary) -> void:
	var record := by_id.get("three_turn_horizon", {}) as Dictionary
	var planner := record.get("planner", {}) as Dictionary
	var oracle := record.get("oracle", {}) as Dictionary
	_check.call("limit_horizon_planner_loses_all", int(planner.get("wins", -1)) == 0 and int(planner.get("losses", -1)) == 6)
	_check.call("limit_horizon_oracle_wins_all", int(oracle.get("wins", -1)) == 6 and int(oracle.get("losses", -1)) == 0)
	var planner_result := _first_result(record, "planner_matches")
	var oracle_result := _first_result(record, "oracle_matches")
	_check.call("limit_horizon_planner_opens_damage", _first_candidate_move(planner_result) == H_STRIKE)
	_check.call("limit_horizon_oracle_opens_speed_setup", _first_candidate_move(oracle_result) == H_SPEED_SETUP)
	_check.call("limit_horizon_oracle_uses_focus_second", _candidate_move_at(oracle_result, 1) == H_ATTACK_SETUP)
	_check.call("limit_horizon_oracle_finishes_with_strike", _candidate_move_at(oracle_result, 2) == H_STRIKE)
	var trace_json := JSON.stringify(_first_candidate_trace(planner_result))
	_check.call("limit_horizon_search_completed_depth_two", trace_json.contains("\"fully_completed_depth\":2"))
	_check.call("limit_horizon_not_budget_exhausted", trace_json.contains("\"budget_exhausted\":false"))
...
func _test_branching_limit(by_id: Dictionary) -> void:
	...
	var planner_result := _first_result(record, "planner_matches")
	var oracle_result := _first_result(record, "oracle_matches")
	_check.call("limit_branch_planner_opens_strike", _first_candidate_move(planner_result) == B_STRIKE)
	_check.call("limit_branch_oracle_opens_debuff", _first_candidate_move(oracle_result) == B_DEBUFF)
...
func _test_hidden_information_limit(by_id: Dictionary) -> void:
	...
	_check.call("limit_hidden_planner_opens_strike", _first_candidate_move(result) == U_STRIKE)
...
func _scenarios() -> Array[Dictionary]:
	...
	return [
		{
			"id": "three_turn_horizon",
			"family": "horizon_beyond_depth_two",
...
func _add_damage_move(id: StringName, power: int) -> void:
	...
	move.accuracy = 100
	move.pp = 20
	# Neutralize the ruleset's base critical chance so limit fixtures are not crit-sensitive.
...
func _first_candidate_move(result: Dictionary) -> StringName:
	return _candidate_move_at(result, 0)
...
func _candidate_move_at(result: Dictionary, turn_index: int) -> StringName:
	...
	var action := turn.get("side_a_action", {}) as Dictionary
