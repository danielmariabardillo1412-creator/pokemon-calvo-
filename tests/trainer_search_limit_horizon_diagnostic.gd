...
func _run() -> void:
	var suite := TrainerSearchLimitBenchmarkTestSuite.new()
	...
	for value in result.get("scenarios", []):
		...
		if String(record.get("id", "")) == "three_turn_horizon":
			horizon = record
			break
	print("HORIZON_DIAGNOSTIC_SUMMARY %s" % JSON.stringify(horizon.get("planner", {})))
	...
	for i in range(matches.size()):
		...
		print("HORIZON_DIAGNOSTIC_MATCH index=%d seed=%d mirrored=%s outcome=%s winner=%s turns=%d actions=%s" % [
			i,
