...
old = 'const COMPLETED_DEFEAT := &"DEFEAT"\n'
new = old + '\nconst SUBSTITUTION_READY := "SUBSTITUTION_READY"\nconst SUBSTITUTION_BLOCKED := "BLOCKED"\n'
assert s.count(old) == 1
s = s.replace(old, new, 1)

old = 'var last_trainer_action_proposal_report: Dictionary = {}\n'
new = old + 'var last_trainer_action_substitution_report: Dictionary = {}\n'
assert s.count(old) == 1
s = s.replace(old, new, 1)

old = 'var _trainer_action_proposal_enabled: bool = false\n'
new = old + 'var _trainer_action_substitution_enabled: bool = false\n'
assert s.count(old) == 1
s = s.replace(old, new, 1)
...
# C3f-ak authoritative substitution toggle. Independent from proposal telemetry and
...
# Narrow side_b contract validator. Only a current, complete, uniquely resolved C3f-aj
# proposal may cross the authoritative boundary, and exact legality is revalidated live.
...
s = p.read_text()
old = '\tTrainerBattleSessionActionProposalAuditTestSuite.new().run(Callable(self, "_check"))\n'
new = old + '\tTrainerBattleSessionAuthoritativeSubstitutionAuditTestSuite.new().run(Callable(self, "_check"))\n'
assert s.count(old) == 1
