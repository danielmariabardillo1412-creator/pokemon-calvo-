...
The first notebook-bearing candidate `05169ac4d6973b63a5dc02a84275230c8ed01a1a` completed 18/18 SUCCESS, but while attempting to close the PR an operator/tool-routing mistake wrote to this notebook instead of calling the PR-close action. That accidental commit truncated this file and moved the branch SHA after certification.

Rules applied:
- the previously green SHA is **not** reused as the final certified snapshot after the branch moved;
- no code, tests or data were changed by the incident;
- this notebook is restored in full and the incident is recorded here;
- engineering → restored final must still contain only notebooks;
- the restored exact HEAD must pass a fresh 18/18 before #91 may close.

## Final certification protocol
After this restored notebook plus synchronized `01_PROJECT_STATE.md` and `04_NEXT_STEPS.md`:
1. engineering `a14b761...` → restored final HEAD must contain only `01`, `04`, `21` notebook files;
2. no code/test/data changes after engineering certification;
3. require a fresh **18/18 SUCCESS** on exact restored notebook-bearing HEAD;
4. close PR #91 without merge using the PR action, not a file action;
