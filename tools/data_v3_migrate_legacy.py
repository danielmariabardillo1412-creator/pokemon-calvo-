...
def gd(text: str) -> str:
    """Dedent Python literal and convert 4-space GDScript indentation to tabs."""
...
def replace_function(text: str, name: str, replacement: str) -> str:
    ...
    replacement = gd(replacement)
    m = matches[0]
    return text[: m.start()] + replacement + "\n\n" + text[m.end() :]


def migrate_test_runner() -> None:
    p = ROOT / "tests/test_runner.gd"
...
def migrate_progression() -> None:
    p = ROOT / "tests/progression_test_suite.gd"
    text = p.read_text(encoding="utf-8")
    replacements = {
        "_test_learnset_queries": '''
        ...
        ''',
        "_test_gain_move_choice_required": '''
        ...
        ''',
        "_test_evolution_coverage_invariant": '''
    ...
    marker = "func _four_valid_moves_excluding(exclude: StringName) -> Array[StringName]:"
    if "func _level_up_move_level(" not in text:
        helper = gd('''
        ...
        if marker not in text:
            raise RuntimeError("progression helper marker not found")
...
def main() -> None:
    migrate_test_runner()
    migrate_progression()
    print("DATA V3 legacy regression migration prepared")
